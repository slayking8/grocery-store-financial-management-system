...
A phone at 320 × 667 CSS pixels can have very little space left when the keyboard includes clipboard and password suggestion rows. Opening quantity entry now focuses the product heading rather than the input. The owner can use +/− without opening the keyboard, or tap the quantity to type.
